...
/**
 * Search babelio for a book and display the list of results. Use background tasks to get thumbnails and update when retrieved.
 * 
 * @author Philip Warner
 */
public class BabelioSearchResults extends BookCatalogueListActivity {
	...
	private ArrayList<BabelioWork> mList = new ArrayList<BabelioWork>();
	private ArrayAdapter<BabelioWork> mAdapter = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mDbHelper.open();
		setContentView(R.layout.babelio_work_list);
	...
	private void doSearch() {
		...
		SearchBooksApiHandler searcher = new SearchBooksApiHandler(baMgr);

		// Run the search
		ArrayList<BabelioWork> works = null;
		try {
			works = searcher.search(mCriteria);
		} catch (Exception e) {
			Logger.logError(e, "Failed when searching babelio");
	...
	private void doItemClick(View v) {
		...
		//Intent i = new Intent(this, BabelioW)
	}

	/**
	 * ArrayAdapter that uses holder pattern to display babelio books and allows for background image retrieval.
	 * 
	 * @author Philip Warner
	 *
	 */
	private class ResultsAdapter extends ArrayAdapter<BabelioWork> {
		/** Used in building views when needed */
		LayoutInflater mInflater;

		public ResultsAdapter() {
			super(BabelioSearchResults.this, 0, mList);
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(convertView == null) {
				...
				try {
					...
					convertView = mInflater.inflate(R.layout.babelio_work_item, parent, false);
