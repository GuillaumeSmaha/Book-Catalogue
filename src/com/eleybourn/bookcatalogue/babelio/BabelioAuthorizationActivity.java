 ...
 */
public class BabelioAuthorizationActivity extends BookCatalogueActivity {
	@Override
	public void onCreate(Bundle savedInstanceState) {
		...
		if (uri != null) {// && uri.toString().startsWith("BookCatalogue")) {  
			// Babelio does not set the verifier...but we may as well check for it.
			// The verifier was added in API version 1.0A, and Babelio seems to 
			// implement 1.0.
			
		    // Handle the auth response by passing it off to a background task to check.
		    BabelioAuthorizationResultCheck task = new BabelioAuthorizationResultCheck();
