...
/**
 * Background task class to send all books in the database to babelio.
 * 
 * @author Philip Warner
 */
public class BabelioSendAllBooksTask extends BabelioGenericTask {

	private static final long serialVersionUID = -3702588929023247618L;
	/** Last book ID processed */
	private long mLastId = 0;
	/** Flag indicating if it should only send UPDATED books to babelio; false == all books */
	 ...
	 * Constructor
	 */
	public BabelioSendAllBooksTask(boolean updatesOnly) {
		super(BookCatalogueApp.getResourceString(R.string.send_books_to_babelio));
	...
	public boolean sendAllBooks(QueueManager qmanager, Context context) throws NotAuthorizedException {
		...
		try {
			books = dbHelper.getAllBooksForBabelioCursor(mLastId, mUpdatesOnly);
		...
		// Notify the user: '15 books processed: 3 sent successfully, 5 with no ISBN and 7 with ISBN but not found in babelio'
		String s = context.getString(R.string.send_all_to_babelio_result, mCount, mSent, mNoIsbn, mNotFound);
		BookCatalogueApp.showNotification(R.id.NOTIFICATION, 
							context.getString(R.string.send_books_to_babelio), s, 
	...
	public long getCategory() {
		return BcQueueManager.CAT_BABELIO_EXPORT_ALL;
	}
}
