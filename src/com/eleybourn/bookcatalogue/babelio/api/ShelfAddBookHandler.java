 ...
 * 		 	babelio are much more restrictive: no spaces, punctuation (at least).
 * 
...
public class ShelfAddBookHandler extends ApiHandler {
	...
	public ShelfAddBookHandler(BabelioManager manager) {
		super(manager);
		buildFilters();
	}
	 ...
	 */
	public long add(String shelfName, long baBookId) 
			throws IOException, NotAuthorizedException, BookNotFoundException, NetworkException
	{
        return doCall(shelfName, baBookId, false);
	}

	/**
	 * Remove the passed book from the passed shelf
	 */	
	public long remove(String shelfName, long baBookId) 
			throws ClientProtocolException, IOException, 
			NotAuthorizedException, BookNotFoundException, NetworkException
	{
        return doCall(shelfName, baBookId, true);
	}

	/**
	 * Do the main work; same API call for add & remove
	 */
	private long doCall(String shelfName, long baBookId, boolean isRemove) 
			throws ClientProtocolException, IOException, 
			NotAuthorizedException, BookNotFoundException, NetworkException
	{
		mReviewId = 0;

		HttpPost post = new HttpPost("http://www.babelio.com/shelf/add_to_shelf.xml");

        List<NameValuePair> parameters = new ArrayList<NameValuePair>();
        if (isRemove)
            parameters.add(new BasicNameValuePair("a", "remove"));
        parameters.add(new BasicNameValuePair("book_id", Long.toString(baBookId)));
