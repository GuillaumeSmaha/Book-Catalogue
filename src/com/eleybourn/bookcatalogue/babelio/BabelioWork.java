...
/**
 * Class to store the 'work' data returned via a babelio search. It also creates a background task
 * to find images and waits for completion.
 * 
 * @author Philip Warner
 */
public class BabelioWork {
	public String title;
	...
	private BabelioGetImageTask mTask;
	...
	public void handleTaskFinished(byte[] bytes) {
		...
		if (v != null) {
			synchronized(v) {
				...
				if (((BabelioWork)ViewTagger.getTag(v, R.id.TAG_BABELIO_WORK)).equals(this)) {
		...
		synchronized(this) {
			if (this.imageBytes == null) {
				...
				if (mTask == null) {
					...
					try {
						mTask = new BabelioGetImageTask(getBestUrl(), this);
						queue.enqueue(mTask);
					} catch (Exception e) {
						Logger.logError(e, "Failed to create task to get image from babelio");
					}
				}
				// Save the work in the View for verification
				ViewTagger.setTag(v, R.id.TAG_BABELIO_WORK, this);
				...
				ViewTagger.setTag(v, R.id.TAG_BABELIO_WORK, null);
			}
		}
	}
