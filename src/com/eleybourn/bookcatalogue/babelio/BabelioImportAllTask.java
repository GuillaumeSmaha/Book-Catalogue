...
import com.eleybourn.bookcatalogue.babelio.api.ListReviewsApiHandler;
import com.eleybourn.bookcatalogue.babelio.api.ListReviewsApiHandler.ListReviewsFieldNames;
import com.eleybourn.bookcatalogue.utils.Logger;
import com.eleybourn.bookcatalogue.utils.Utils;

/**
 * Import all a users 'reviews' from babelio; a users 'reviews' consistes of all the books that 
 * they have placed on bookshelves, irrespective of whether they have rated or reviewd the book.
 * 
 * @author Philip Warner
 */
public class BabelioImportAllTask extends BabelioGenericTask {

	private static final long serialVersionUID = -4341468279159737349L;
	...
	/** Lookup table of bookshelves defined currently and their babelio canonical names */
	 ...
	 * Constructor
	 */
	public BabelioImportAllTask(boolean isSync) {
		super(BookCatalogueApp.getResourceString(R.string.import_all_from_babelio));
		mPosition = 0;
		mIsSync = isSync;
		...
		if (mIsSync) {
			Date lastSync = BabelioManager.getLastSyncDate();
			if (lastSync == null) {
				mUpdatesAfter = null;
			} else {
	...
	public boolean run(QueueManager qMgr, Context context) {
		...
		try {
			// Load the babelio reviews
			boolean ok = processReviews(qMgr, db);
			// If it's a sync job, then start the 'send' part and save last syn date
			if (mIsSync) {
				BabelioManager.setLastSyncDate(mStartDate);
				QueueManager.getQueueManager().enqueueTask(new BabelioSendAllBooksTask(true), BcQueueManager.QUEUE_MAIN, 0);
			}
			return ok;
		} finally {
			if (db != null)
	...
	private boolean processReviews(QueueManager qMgr, CatalogueDBAdapter db) {
		BabelioManager gr = new BabelioManager(true);
	...
	private void processReview(CatalogueDBAdapter db, Bundle review) {
		long baId = review.getLong(ListReviewsFieldNames.BA_BOOK_ID);

		// Find the books in our database - NOTE: may be more than one!
		// First look by babelio book ID
		BooksCursor c = db.fetchBooksByBabelioBookId(baId);
		try {
	...
	/** 
	 * Passed a babelio shelf name, return the best macthing localk bookshelf name, or the
	 * original if no match found.
	 * 
	 * @param db			Database adapter
	 * @param grShelfName	Babelio shelf name
	 * 
	 * @return				Local name, or babelio name if no match
	 */
		...
		if (mBookshelfLookup == null) {
			...
			try {
				...
				while (c.moveToNext()) {
					...
					mBookshelfLookup.put(BabelioManager.canonicalizeBookshelfName(name), name);
				}
			} finally {
				if (c != null)
					c.close();
	...
	private void updateBook(CatalogueDBAdapter db, BooksRowView rv, Bundle review) {
		...
		final String lastGrSync = rv.getString(DOM_LAST_BABELIO_SYNC_DATE.name);
		...
		//db.setBabelioSyncDate(rv.getId());
	}
	...
	private void createBook(CatalogueDBAdapter db, Bundle review) {
		...
		//db.setBabelioSyncDate(id);
	}

	/**
	 * Build a book bundle based on the babelio 'review' data. Some data is just copied
	...
	private BookData buildBundle(CatalogueDBAdapter db, BooksRowView rv, Bundle review) {
		...
		addStringIfNonBlank(review, ListReviewsFieldNames.DB_TITLE, book, ListReviewsFieldNames.DB_TITLE);
		addLongIfPresent(review, ListReviewsFieldNames.BA_BOOK_ID, book, DOM_BABELIO_BOOK_ID.name);
        ...
        String pubDate = BabelioManager.buildDate(review, ListReviewsFieldNames.PUB_YEAR, ListReviewsFieldNames.PUB_MONTH, ListReviewsFieldNames.PUB_DAY, null);
        ...
        Date now = new Date();
        book.putString(DOM_LAST_BABELIO_SYNC_DATE.name, Utils.toSqlDateTime(now));
	...
	public long getCategory() {
		return BcQueueManager.CAT_BABELIO_IMPORT_ALL;
	}
